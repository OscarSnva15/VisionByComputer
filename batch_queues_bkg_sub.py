...
def compute_video_background( video_to_process ):
    ...
    if not captured_video.isOpened:
        ...
        return '%s says ERROR %s %s on generate %s' % \
        (current_process().name, compute_video_background.__name__, video_to_process, out_background_filename)
    ...
    return '%s says that %s %s generate %s' % \
        (current_process().name, compute_video_background.__name__, video_to_process, out_background_filename)
